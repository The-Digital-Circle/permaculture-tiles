...
def synthetic_geodata():
    ...
    river, one urban footprint. Land starts a touch east of the x=0 tile seam so the western ocean
    column prunes cleanly (avoids an all_touched stray pixel exactly on the tile boundary)."""
    O = geo.ORIGIN
    ...
    urban = gpd.GeoDataFrame(geometry=[box(O * 0.6, O * 0.1, O * 0.7, O * 0.2)], crs=3857)
